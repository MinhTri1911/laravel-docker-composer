...
var billingPaper = new function () {
  ...
  this.models = {
    ...
    // Info login
    loginOpeCompanyId: '#login-ope-company-id',
    loginAuthOperation: '#login-auth-operation',
  ...
  this.init = function () {
    ...
    $(document).on('click', billingPaper.models.lblRadioCompany, function () {
      billingPaper.events.clickRadioButtonRow(this);
    });
  ...
  this.events = {
    ...
    /**
     * Event click button radio button on row
     *
     * @returns void
     */
    clickRadioButtonRow: function ($el) {
      var localStorage = JSON.parse(window.localStorage.getItem(ID_SCREEN));
      var btnRadioId = '#' + $($el).attr('for');

      localStorage['historyBillingId'] = parseInt($(btnRadioId).attr('hb-id'));
      localStorage['companyId'] = parseInt($(btnRadioId).val());
      window.localStorage.setItem(ID_SCREEN, JSON.stringify(localStorage));

      // Check permission button
      billingPaper.events.checkPermissionButton(btnRadioId);
    },
    ...
    displayProcessBilling: function () {
      var status = parseInt($(billingPaper.models.sltStatus).val());
      var approve = parseInt($(billingPaper.models.sltApprove).val());
    ...
    autoCheckedRadioCompany: function (companyId) {
      ...
      if (companyId !== undefined && companyId !== null) {
        $(billingPaper.models.btnRadioCompany).each(function () {
          if (companyId === parseInt($(this).val())) {
    ...
    createBillingPaper: function () {
      ...
      // Get companyId
      var companyId = parseInt($("input:radio[name='rdo_company']:checked").val());
    ...
    deliveryBillingPaper: function () {
      ...
      if (DELIVERIED === parseInt($(billingPaper.models.statusBilling + companyId).val())) {
    ...
    refreshLocalStore: function (){
      ...
      localStorage['companyId'] = -1;
      localStorage['historyBillingId'] = -1;
      window.localStorage.setItem(ID_SCREEN, JSON.stringify(localStorage));
    },
     ...
     * Check permission button
     * 
     * @param $el radio hidden button company
     * @returns void
     */
    checkPermissionButton: function ($btnRadio){

      // Check permission operation
      if (!parseInt($(billingPaper.models.loginAuthOperation).val())) {

        // Get operation company Id of record
        var opeCompanyId = parseInt($($btnRadio).attr('ope-id'));

        // Check have permisssion
        if (opeCompanyId === parseInt($(billingPaper.models.loginOpeCompanyId ).val())) {
          $(billingPaper.models.btnCreate).attr('style', '');
          $(billingPaper.models.btnDelivery).attr('style', '');
        } else {
          $(billingPaper.models.btnCreate).attr('style', 'display: none;');
          $(billingPaper.models.btnDelivery).attr('style', 'display: none;');
        }
      }
    }
  };

...
class CreateCompanyRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        ...
        $checkNationShipExists = $nationRepository->where('id', $this->get('ship-nation-id'))->exists();

        if (!$checkNationShipExists) {
            $this->_addValidatorMessage($validator, 'ship-nation-id', trans('common-message.error.E009', [
                    'value' => trans('ship.lbl_title_nation')
                ])
            );
        }

        $classShipRepository = app(\App\Repositories\Classification\ClassificationInterface::class);
        $checkClassShipExists = $classShipRepository->where('id', $this->get('slb-ship-classification'))->exists();

        if (!$checkClassShipExists) {
            $this->_addValidatorMessage($validator, 'slb-ship-classification', trans('common-message.error.E009', [
                    'value' => trans('ship.lbl_title_classification')
                ])
            );
        }

        $shipTypeRepository = app(\App\Repositories\ShipType\ShipTypeInterface::class);
        $checkShipTypeExists = $shipTypeRepository->where('id', $this->get('slb-ship-type'))->exists();

        if (!$checkShipTypeExists) {
            $this->_addValidatorMessage($validator, 'slb-ship-type', trans('common-message.error.E009', [
                    'value' => trans('ship.lbl_title_ship_type')
                ])
            );
        }
    }

...
class CompanyRepository extends EloquentRepository implements CompanyInterface
{
     ...
     * @param int $showType select company have contract active and inverse
     * @return mixed
     */
    public function getListCompanyCommon($groupType = 0, $showType = 0)
    {
        // Check group type and set subquery for count total license
        if (!$groupType) {
            $subQuery = ($showType == 0 || !$showType) 
    ...
    private function _joinWithShowType($subQuery, $groupType = 0, $showType = 0)
    {
        if (!$showType) {
